...
/*
================================================== 
This file is a part of CMSimple 5.20
Released: 2025-07-17
...
function evaluate_plugincall($__text) 
{
	...
	$pl_calls = array('plugin:' => 'return {{%1}}', 'function:' => 'return {{%1}}', 'f:' => 'return {{%1}}', 'p:' => 'return {{%1}}');
...
function final_clean_up($html) 
{
    ...
    if ($adm === true) 
    {
        ...
        if(count($errors) > 0)
        {
            ...
            $errorList .= '
<div class="cmsimplecore_warning" style="padding-top: 60px; margin: 0; border-width: 0; border-bottom-width: 3px;">
<ul>
';
...
{
	global $adm, $cf, $csrfSession;
	if($adm && $cf['use']['csrf_protection'] == 'true')
	{
		if($_POST['csrf_token'] !== $_SESSION[$csrfSession])die("<a href=\"./?logout\">CSRF protection in action - logout &raquo;</a>");
	}
}
...
{
    global $download, $tx;
    if (!is_readable($fl) || ($download != '' && !chkdl('./?download=' . basename($fl)))) 
	{
        global $o, $text_title;
        shead('404');
...
{
    global $e, $tx;
    $e .= '<li><b>' . $tx['error'][$et] . ' ' . $tx['filetype'][$ft] . '</b><br>' . $fn . '</li>' . "\n";
}
...
function a($i, $x) 
{
    ...
    return isset($u[$i]) ? '<a href="./?' . $u[$i] . $x . '">' : '<a href="./?' . $x . '">';
}
...
function meta($n) 
{
    ...
    if ($cf['meta'][$n] != '' && !($print && in_array($n, $exclude)))
        return '<meta name="' . $n . '" content="' . htmlspecialchars($cf['meta'][$n], ENT_COMPAT, 'UTF-8') . '">' . "\n";
}
...
function ml($i, $img_src = '') 
{
    ...
    $t = '';
    $title = $tx['menu'][$i];
    if ($f != $i) $t .= '<a href="./?'.amp().$i.'">';
    if ($img_src != '') 
	{
        $t .= '<img title="' . $title . '" src="' . $pth['folder']['template'] . 'images/' . $img_src . '" alt="' . $title . '">';
    }
    else 
	{ 
		$t .= $title; 
...
function homelink($img_src = '') 
{
    global $pth, $s, $u;
    $homepage = $u[0];
    $img = '<img title="' . $homepage . '" src="' . $pth['folder']['template'] . 'images/' . $img_src . '" alt="' . $homepage . '">';
    if ($s == 0) $t = $img;
    else $t = a(0, '') . $img . '</a>';
    return $t;
} 
...
function head() 
{
	...
	{
		$t.='<link rel="shortcut icon" type="image/x-icon" href="./favicon.ico">' . "\n";
	}
	if(file_exists('./favicon.gif'))$t.='<link rel="icon" type="image/gif" href="./favicon.gif">' . "\n";
	if(file_exists('./favicon.png'))$t.='<link rel="icon" type="image/png" href="./favicon.png">' . "\n";

    $t = '<meta charset="utf-8">' . "\n" . $t;
    return $t . '<meta name="generator" content="' . CMSIMPLE_VERSION . ' ' . CMSIMPLE_RELEASE . ' - www.cmsimple.org">' . "\n" . '<link rel="stylesheet" href="' . $pth['file']['corestyle'] . '" type="text/css">' . "\n" . '<link rel="stylesheet" href="' . $pth['file']['stylesheet'] . '" type="text/css">' . "\n" . $hjs;
}
...
function pagename() 
{
    ...
    if($s>=0) return($pd_current['show_heading'] == '1' ? htmlspecialchars(strip_tags($pd_current['heading']), ENT_QUOTES, 'UTF-8'): $h[$s]);
}
...
function li($ta, $st) 
{
	...
	if ($st == 'submenu' || $st == 'search')
		$t .= '<ul class="' . $st . '">' . "\n";
	$b = 0;
	if ($st > 0 && $st != 'sitemaplevel' && $st != 'menulevel' && $st != 'submenu') 
	{
		$b = intval($st)-1;
		$st = 'menulevel';
	}
...
function printlink($img_src = '') 
{
    ...
    if ($img_src != '') 
	{
        $link = '<img title="' . $title . '" src="' . $pth['folder']['template'] . 'images/' . $img_src . '" alt="' . $title . '">';
    }
    else 
	{ 
		$link .= $title; 
...
function cmsimple_img($src='',$class='',$style='',$alt='') 
{
	global $pth;
	return '<img src="' . $pth['folder']['images'] . $src . '" alt="' . $alt . '" style="' . $style . '" class="' . $class . '">';
}

function cmsimple_video($src='',$class='',$style='') 
{
	global $pth;
	return '<video src="' . $pth['folder']['videos'] . $src . '" class="' . $class . '" style="' . $style . '" controls></video>';
}

function cmsimple_audio($src='',$class='',$style='') 
{
	global $pth;
	return '<audio src="' . $pth['folder']['audios'] . $src . '" class="' . $class . '" style="' . $style . '" controls></audio>';
}

function opendiv($class='',$style='',$id='') 
{
